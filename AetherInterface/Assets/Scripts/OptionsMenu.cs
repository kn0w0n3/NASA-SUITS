...
public class OptionsMenu : AetherMenu {
    ...
    public void ChangeListHeight(int count)
    {
        GameObject goButton = (GameObject)Instantiate(Resources.Load("Button"));
        float height = goButton.GetComponent<RectTransform>().rect.height;
        RectTransform f = content.GetComponent<RectTransform>();
        // number of elements * height of the buttons + num of elements * spacing between buttons + 20 for the default padding
        float newH = ((count * height) + (count * 5)) + 20;
        //Debug.Log(count );
        f.sizeDelta = new Vector3(0, newH, 0);
    }
